...
nathan = User.create!(email: 'NathanMenard.work@gmail.com', password: "secret", first_name: 'Nathan', last_name: 'Menard', profile_picture: 'https://avatars0.githubusercontent.com/u/49595305?s=460&v=4')
maella = User.create!(email: 'mdegras@myrezapp.com', password: "secret", first_name: 'Maella', last_name: 'Degras', profile_picture: 'https://avatars2.githubusercontent.com/u/6984625?s=460&v=4')
...
nathan_pool = Pool.create!(user: nathan, address: '12 rue Legraverend, Rennes', capacity: 15, title: 'A pool for all your family', description: description_lorem, heated: false, picture: File.open(Rails.root.join('db/fixtures/pools/piscine1.jpg')), daily_price: 15)
maella_pool = Pool.create!(user: maella, address: '55 rue du Faubourg Saint-Honoré, Paris', capacity: 10, title: 'A great pool close to the forest', description: description_lorem, heated: true, picture: File.open(Rails.root.join('db/fixtures/pools/pool.jpg')), daily_price: 20)
simon_pool = Pool.create!(user: simon, address: '1 rue de la gare, Rennes', capacity: 8, title: 'A nice and heated pool', description: description_lorem, heated: true, picture: File.open(Rails.root.join('db/fixtures/pools/pool4.jpg')), daily_price: 10)
valentin_pool = Pool.create!(user: valentin, address: '96 rue broca, Paris',capacity: 20, title: 'A pool close to the town', description: description_lorem, heated: true, picture: File.open(Rails.root.join('db/fixtures/pools/piscine3.jpg')), daily_price: 25)
maxime_pool = Pool.create!(user: maxime, address: '1 rue de Fougères, Rennes', capacity: 17, title: 'Welcome to my awesome pool', description: description_lorem, heated: false, picture: File.open(Rails.root.join('db/fixtures/pools/piscine4.jpg')), daily_price: 17)
aristide_pool = Pool.create!(user: aristide, address: '5 rue des Pyrénées, Paris', capacity: 20, title: 'In Paris, come enjoy my pool', description: description_lorem, heated: true, picture: File.open(Rails.root.join('db/fixtures/pools/pool2.jpg')), daily_price: 12)
cecile_pool = Pool.create!(user: cecile, address: '1 boulevard de la duchesse anne, Rennes', capacity: 10, title: 'Closed to Rennes, a great pool', description: description_lorem, heated: false, picture: File.open(Rails.root.join('db/fixtures/pools/piscine6.jpg')), daily_price: 30)
